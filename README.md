...
| <img width="442" height="218" alt="image" src="https://github.com/user-attachments/assets/c47dc9f5-c64b-4803-9918-8bc38c415c0f" /> | 
| :---: | 
| 「。、。V12」で、V1 を使用した際の結果ダイアログ。 |
...
| <img width="292" height="201" alt="image" src="https://github.com/user-attachments/assets/f86a666e-96ea-4f99-8214-0223b028b87d" /> | 
| :---: | 
| V12 選択画面 |
...
```
javascript:(function(){var s=document.createElement('script');s.src='//cdn.jsdelivr.net/gh/weepjp/syokaklet/v12.js?_='+Date.now();document.body.appendChild(s);})();
```
...
* 末尾の特定範囲において、以下のワードが含まれていると切り落としされます。
  * 切り落とした場合はダイアログに「`※カット有（判定ワード）`」と出ます。
...
* えっ？　そこから？
* 何のために「検索サイト」とか「AIチャット」が、存在するのではないのでしょうか？
...
* ブラウザのアドレスバーや別処理にカーソルを置いた状態であるか、広告かアクセス解析等が JavaScript を処理し、現在地の URL なんかこそっと取得してるタイミングと重なった時にありがちです。<s>広告と解析うぜー。。</s>。
* こちらとしては、「予期しないエラー」なので知りません（たまにこういう時もあるということで、気にしないで下さい）。
* 取得先ページとカーソルの位置をよ～～く確認してから、再度チャレンジしてみてください。 
...
* VOICEROID2 は、<s>時代錯誤のアプリ</s> Shift_JIS 環境なので、Unicode固有のJIS第3・第4水準の異体字は〓(下駄文字)となり、読み上げや読み設定に対応しないので対策。
